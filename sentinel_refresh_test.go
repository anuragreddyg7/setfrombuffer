	...
	s0 := &mockConn{
		...
		DoMultiFn: func(multi ...Completed) *valkeyresults {
			...
			return &valkeyresults{s: []ValkeyResult{
				...
				{val: slicemsg('*', []ValkeyMessage{strmsg('+', ""), strmsg('+', addr)})},
			}}
		},
	}
...
func TestSentinelTopologyRefreshKeepsReplica(t *testing.T) {
	...
	replicasReply := func() ValkeyResult {
		out := make([]ValkeyMessage, 0, len(replicaAddrs))
		for _, addr := range replicaAddrs {
			fields := []ValkeyMessage{
				strmsg('+', "ip"), strmsg('+', ""),
		...
		return ValkeyResult{val: slicemsg('*', out)}
	}
